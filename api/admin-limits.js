...
// Ajusta os limites de vagas de um curso (protegido pela senha do painel).
module.exports = async (req, res) => {
  ...
  const { data, error } = await supabase.rpc('ftrails_set_limites', { p_curso: curso, p_limite_unb: limUnb, p_limite_externo: limExt });
  if (error) { console.error('set_limites error', error); return res.status(500).json({ error: 'falha' }); }

  return res.status(200).json({ status: 'ok', curso, limite_unb: data.limite_unb, limite_externo: data.limite_externo });
};
