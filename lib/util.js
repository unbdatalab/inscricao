...
// Resolve o curso a operar: usa o informado (se existir na config) ou o curso ativo (público).
async function resolveCurso(supabase, wanted) {
  const w = String(wanted || '').trim();
  const { data } = await supabase.from('ftrails_config').select('curso,ativo,ordem');
  const list = data || [];
  if (w && list.some(c => c.curso === w)) return w;
  const ativo = list.filter(c => c.ativo).sort((a, b) => (a.ordem || 99) - (b.ordem || 99))[0];
  if (ativo) return ativo.curso;
  const first = list.slice().sort((a, b) => (a.ordem || 99) - (b.ordem || 99))[0];
  return (first && first.curso) || 'gestao';
}
...
// Aceita qualquer e-mail válido. @unb.br (e subdomínios) => vaga interna (UnB);
// qualquer outro e-mail válido => vaga externa (incl. Gmail/Outlook).
...
const VINCULOS = ['docente','tae','docente-gestor','externo','egresso','outro'];
...
function confirmedHtml(nome, categoria, cursoNome) {
  const vaga = categoria === 'unb' ? 'vaga interna (UnB)' : 'vaga externa';
  const curso = cursoNome ? `<strong>${escapeHtml(cursoNome)}</strong>` : 'que você escolheu na <strong>Trilha FTrails</strong>';
  ...
  <div style="max-width:560px;margin:0 auto;padding:24px">
    ...
    <div style="background:#fff;padding:26px;border-radius:0 0 14px 14px;border:1px solid #e3e9f4;border-top:none">
      ...
      <p>Sua inscrição no curso ${curso} foi <strong>confirmada</strong> pela coordenação, na sua ${vaga}.</p>
      <p style="margin-top:18px">As sessões acontecem de forma remota. Você receberá da coordenação as orientações de acesso (datas, sala virtual e ambiente Aprender3) antes da primeira sessão. Até lá!</p>
...
async function sendConfirmed(to, nome, categoria, cursoNome) {
  ...
  await t.sendMail({ from, to, subject: 'Inscrição confirmada — Trilha FTrails · IA (DataLab/UnB)', html: confirmedHtml(nome, categoria, cursoNome) });
  return { sent: true };
}
...
function messageHtml(nome, corpo) {
  ...
  <div style="max-width:560px;margin:0 auto;padding:24px">
    <div style="background:#12356b;color:#fff;padding:20px 26px;border-radius:14px 14px 0 0">
      <div style="font-size:13px;letter-spacing:.5px;opacity:.9">TRILHA FTRAILS · DATALAB · FACULDADE DE TECNOLOGIA · UnB</div>
      <div style="font-size:19px;font-weight:700;margin-top:6px">Trilha FTrails · Inteligência Artificial</div>
    </div>
    <div style="background:#fff;padding:26px;border-radius:0 0 14px 14px;border:1px solid #e3e9f4;border-top:none">
      <p>Olá, <strong>${escapeHtml(nome || '')}</strong>.</p>
...
module.exports = { getSupabase, resolveCurso, checkInstitutionalEmail, isValidCPF, UFS, VINCULOS, sendConfirmed, sendMessage, smtpReady };
